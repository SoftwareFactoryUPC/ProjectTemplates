...
class Empresa extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('panel/empresa_model','empresa');
	...
	public function listado()
	{
		...
		if(!$auth->check(FALSE)){
			...
			$data['empresa']  =  $this->empresa->get_listado();

			$this->template->display('panel/empresa/listado', $data);
		}
	}
	...
	public function nuevo()
	{
		...
		$this->template->display('panel/empresa/nuevo', $data);
	}
	...
	public function agregar()
	{
		...
		if(!$auth->check(FALSE)){
			...
			$this->form_validation->set_rules('no_empresa', 'titulo', 'required');
			$this->form_validation->set_rules('tx_empresa', 'descripción', 'required');
	...
	public function editar($id=0)
	{
		...
		$data['modulos']  = $this->modulo->get_modulos();
		$data['empresa']  = $this->empresa->get_listado_id($id);

		$this->template->display('panel/empresa/editar', $data);
	}
	...
	public function update()
	{
		...
		if(!$auth->check(FALSE)){
			...
			if(!$this->form_validation->run())
			{
				...
				$this->empresa->update($this->_IS_ARCHIVO);
